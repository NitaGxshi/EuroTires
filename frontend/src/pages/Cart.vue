                    ...
                    <div class="in-cart col-md-9">
                        <div class="box">
                            <div class="box-title item-total row">
                                <h3>
                                    <p style="font-size: 15px;">{{ filtertires.length.toString() }}
                                        <span v-if="filtertires.length < 2">item</span>
                            ...
                            <div v-if="!filtertires.length">
                                <div class="box-content row no-tire">
                                    <div class="content">
                            ...
                            <div v-else>
                                <div v-for="(f, index) in filtertires" :key="index">
                                    <div class="box-content row">
                                        ...
                                        <div class="desc col-sm-4">
                                            <h2 class="item-name">{{ f.tire_name }}</h2>
                                            <div class="item-desc">
                                                <b>Description</b>
                                                <p>{{ f.tire_desc }}</p>
                                            </div>
                                        ...
                                        <div class="item-price col-sm-1">
                                            <span class="sale-price">${{ parseFloat(f.tire_price) -
                                                    parseFloat(f.tire_discount)
                                            }}</span>
                                            <p class="text-muted first-price"
                                                v-if="parseFloat(f.tire_discount) != 0.00">
                                                ${{
                                                        parseFloat(f.tire_price)
                                                }}
                        ...
                        <div class="box-content row">
                            ...
                            <button class="btn check-out-btn" style="margin-left: 10px;"
                                :disabled="filtertires.length ? false : true" @click="checkOutBtn()"><i
                    ...
                    <div class="col-md-3">
                        <div class="box">
                            ...
                            <div class="box-content">
                                ...
                                <div class="btn-group">
                                    <button class="btn check-out-btn" :disabled="filtertires.length ? false : true"
                                        @click="checkOutBtn()"><i class="fa fa-shopping-cart"></i>
                                        Checkout</button>
                                    <button class="btn cancel-btn" @click="cancelBtn()"
                                        :disabled="filtertires.length ? false : true">
                                        Cancel</button>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    computed: {
        ...mapState(["alltires", "user"]),

        filtertires: function () {
            return this.alltires.filter(
    ...
    methods: {
        matchID: function (tire, cartArray) {
            let temp = "";
            cartArray.forEach(element => {
                if (parseInt(tire.tire_id) == element) {
                    temp = tire
                }
            });
            return temp
        ...
        calculateItemPrice: function (index) {
            return ((parseInt(this.filtertires[index].tire_price) - parseInt(this.filtertires[index].tire_discount)) * this.itemQuantity[index]).toString()
        },
        ...
        calculateSummaryPrice: function () {
            ...
            while (i < this.itemQuantity.length) {
                subtotal = subtotal + parseInt(this.filtertires[i].tire_price) * this.itemQuantity[i]
                discount = discount + parseInt(this.filtertires[i].tire_discount) * this.itemQuantity[i]
                i = i + 1
            }
            if (!this.filtertires.length) {
                delivery = 0
            }
        ...
        async onQtyChange(e, i) {
            ...
            let data = {
                ...
                tire_id: parseInt(this.cartItem[i]),
        ...
        async getAllCartItem() {
            if (this.user) {
                ...
                existItem.data.forEach(element => {
                    this.cartItem.push(element.tire_id);
