<template>
    ...
    <section class="order-section">
        ...
        <!-- booking form -->
        <form id="booksessionForm" @submit="handleSubmit" novalidate autocomplete="off">
            ...
            <div class="row">
                ...
                <div class="input-box">
                    <label for="osessions">how many sessions</label>
                    <input type="number" name="osessions" id="osessions" v-model="orderObj.sessions">
                    <p v-if="errorObj.sessionsErr.length > 0">{{ errorObj.sessionsErr[0] }}</p>
                </div>
            </div>
            ...
            <div class="row">
                <div class="input-box">
                    ...
                    <textarea placeholder="your message, do you want to decorate your session?" name="uMessage"
...
export default {
    ...
    data() {
        return {
            orderObj: { name: "", phone: "", people: "", sessions: "", card: "", when: "", note: "" },
            errorObj: { nameErr: [], phoneErr: [], peopleErr: [], sessionsErr: [], cardErr: [], whenErr: [] },
        }
    },
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.orderObj.sessions) {
                this.errorObj.sessionsErr.push("Entering number of sessions is required");
            }
            else {
                if (parseInt(this.orderObj.sessions) > 50) {
                    this.errorObj.sessionsErr.push("Each store can only have maximum 50 sessions");
                }

                if (parseInt(this.orderObj.sessions) < 1) {
                    this.errorObj.sessionsErr.push("Number of sessions must be greater than or equal to 1");
                }

                if (parseInt(this.orderObj.people) < parseInt(this.orderObj.sessions)) {
                    this.errorObj.sessionsErr.push("The number of sessions must be less than the number of people");
                }
            }
        ...
        async handleSubmit(e) {
            ...
            if (!this.checkEmptyErr()) {
                ...
                let data = {
                    ...
                    book_sessions: parseInt(this.orderObj.sessions),
